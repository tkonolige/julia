...
#ifdef _CPU_AARCH64_
#  define ARM_NS AArch64
#else
#  define ARM_NS AArch32
#endif
...
static inline bool str_startswith(const std::string &str, const char *prefix)
{
    return strncmp(str.c_str(), prefix, strlen(prefix)) == 0;
}

template<typename T>
static inline bool read_hexline(const std::string &str, T &out)
{
    if (auto start = strchr(str.c_str(), ':')) {
        start++;
        int val = 0;
        if (sscanf(start, "%i", &val)) {
            out = (T)val;
            return true;
        }
    }
...
static inline void get_cpuinfo_procfs(std::set<CPUID> &res)
{
    ...
    for (std::string line; std::getline(file, line);) {
        ...
        if (str_startswith(line, "CPU implementer")) {
            if (impl)
                reset();
            impl = read_hexline(line, cpuid.implementer);
        }
        if (str_startswith(line, "CPU variant")) {
            if (var)
                reset();
            var = read_hexline(line, cpuid.variant);
        }
        if (str_startswith(line, "CPU part")) {
            if (part)
                reset();
            part = read_hexline(line, cpuid.part);
        }
    }
    reset();
}
...
static std::pair<int,bool> get_elf_arch(void)
{
#ifdef _CPU_AARCH64_
...
#  if __ARM_ARCH > 6 && defined(__ARM_ARCH_PROFILE)
    ism = __ARM_ARCH_PROFILE == 'M';
#  endif
    return std::make_pair(ver, ism);
#endif
}
...
#  if defined(__ARM_ARCH_PROFILE)
    if (arch.first >= 7) {
        ...
        else if (__ARM_ARCH_PROFILE == 'R') {
            set_bit(features, Feature::rclass, true);
        }
        else if (__ARM_ARCH_PROFILE == 'A') {
...
static inline const std::string &host_cpu_name()
{
    static std::string name = [] {
        if (is_generic_cpu_name(host_cpu.first)) {
            auto llvm_name = jl_get_cpu_name_llvm();
            if (llvm_name != "generic") {
                return llvm_name;
            }
        }
        return std::string(find_cpu_name(host_cpu.first));
    }();
    return name;
}
...
#ifdef _CPU_ARM_
        unset_bits(target.features, Feature::neon);
#endif
    }
...
#ifdef _CPU_ARM_
        if (fename.bit == Feature::hwdiv_arm)
            fename_str = "hwdiv-arm";
...
#endif
        if (enable) {
            feature_strs.insert(feature_strs.begin(), fename_str);
        }
        else if (!(data.flags | JL_TARGET_INCOMPLETE_FEATURE)) {
...
{
    return get_llvm_target_str(TargetData<feature_sz>{host_cpu_name().c_str(), "",
                real_feature_masks, 0}, llvmver);
}
...
std::vector<jl_target_spec_t> jl_get_llvm_clone_targets(uint32_t llvmver)
{
    ...
    for (auto &target: jit_targets) {
        ...
        ARM_NS::disable_depends(features);
