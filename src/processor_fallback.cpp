...
static uint32_t sysimg_init_cb(const void *id)
{
    ...
    // Find the first name match or use the default one.
    uint32_t best_idx = 0;
...
std::pair<std::string,std::string> jl_get_llvm_disasm_target(uint32_t)
{
    return get_llvm_target_str(TargetData<1>{host_cpu_name().c_str(),
